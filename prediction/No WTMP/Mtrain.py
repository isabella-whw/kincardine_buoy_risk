...
def main():
    ...
    df = pd.read_csv(os.path.join(BASE_DIR, "AllDat5_ERA5_2023_noWTMP.csv")) #Add path to your data
    predictor_cols = (
    ...
    # response = ["T"]
    response = ["WS"] #Use AllDat5_ERA5_2023_noWTMP.csv as input
    ...
    y = df[response].to_numpy(dtype=float, copy=True).ravel()
    y[(y < -1) | np.isin(y, outDat)] = np.nan

    mask = (~np.isnan(X).any(axis=1)) & (~np.isnan(y)) #Check to see if data is consistent
    X = X[mask]
    y = y[mask]
