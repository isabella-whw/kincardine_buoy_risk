...
from GLRCC import pred_haz


# ------------------------------------------------------------
# Load and prepare data
df = pd.read_csv("NOAA_2019to2024_predictions.csv")
...
def plot_wave_and_hazard(df, year):
    ...
    ax1.set_xlim([pd.Timestamp("2022-09-01 11:00"),
                  pd.Timestamp("2022-09-29 11:00")])
    ax1.set_xlabel('Date')
...
tbl2 = df_full.groupby(["year", "risk_level"]).size().unstack(fill_value=0)
...
tbl3 = df_full.groupby("risk_level")[["wave_height_m", "wind_speed_ms", "wave_period_s"]].mean().round(2)
...
with PdfPages(pdf_filename) as pdf:
    ...
    ymax = max(df_full["total_score"].max(), 9)
    ...
    pair = sns.pairplot(
        df_full,
        vars=["wave_height_m", "wave_period_s", "wind_speed_ms", "total_score"],
        hue="risk_level",
    ...
    fig_year = plot_wave_and_hazard(df_full, 2022)
